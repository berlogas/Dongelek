...
class syncList {
  ...
  _sync(firstSelectId, secondSelectId) {
    ...
    if (firstSelect.length > 0) {
      //если первый (старший) SELECT не пуст
      ...
      var optionData =
        this.dataList[
          firstSelect.options[
            firstSelect.selectedIndex == -1 ? 0 : firstSelect.selectedIndex
          ].value
        ];
      //заполняем второй (подчиненный) селект значениями (создаем элементы option)
      // console.log(optionData, secondSelect.style.display)
      if (optionData) {
      ...
      for (var key in optionData || null)
        secondSelect.options[secondSelect.length] = new Option(
          optionData[key],
          key
        );
      ...
      // if (secondSelect.length === 0) secondSelect.style.display = "none";
    }
