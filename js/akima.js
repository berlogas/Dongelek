...
// }
class NullArgumentException extends Error {}
class DimensionMismatchException extends Error {}
class NumberIsTooSmallException extends Error {}
class OutOfRangeException extends Error {}
class MathInternalError extends Error {}
...
function checkOrder(val, dir, strict, abort) {
...
class SplineInterpolatorAkima {
  ...
  interpolate(xvals, yvals) {
    ...
    if (xvals.length != yvals.length) {
      throw new E.DimensionMismatchException(
        "xvals and yvals arrays have different sizes: xvals.length is " +
          xvals.length +
          " while yvals.length is " +
          yvals.length
      );
    }

    if (xvals.length < this.MINIMUM_NUMBER_POINTS) {
      throw new E.NumberIsTooSmallException(
        "Got " +
          xvals.length +
          " points but I need at least " +
          this.MINIMUM_NUMBER_POINTS
      );
    }
    ...
    for (var i = 2; i < firstDerivatives.length - 2; i++) {
      var wP = weights[i + 1];
      var wM = weights[i - 1];
      if (Math.abs(wP) < Number.EPSILON && Math.abs(wM) < Number.EPSILON) {
        var xv = xvals[i];
        var xvP = xvals[i + 1];
        var xvM = xvals[i - 1];
        firstDerivatives[i] =
    ...
    firstDerivatives[0] = this.differentiateThreePoint(
      xvals,
      yvals,
      0,
      0,
    ...
    firstDerivatives[1] = this.differentiateThreePoint(
      xvals,
      yvals,
      1,
      0,
    ...
    firstDerivatives[xvals.length - 2] = this.differentiateThreePoint(
      xvals,
      yvals,
      xvals.length - 2,
      xvals.length - 3,
    ...
    firstDerivatives[xvals.length - 1] = this.differentiateThreePoint(
      xvals,
      yvals,
      xvals.length - 1,
      xvals.length - 3,
  ...
  interpolateHermiteSorted(xvals, yvals, firstDerivatives) {
    ...
    if (xvals.length != firstDerivatives.length) {
      throw new E.DimensionMismatchException(
        "xvals and yvals arrays are not the same size as firstDerivatives: xvals.length is " +
          xvals.length +
          " while firstDerivatives is " +
          firstDerivatives
      );
    }
